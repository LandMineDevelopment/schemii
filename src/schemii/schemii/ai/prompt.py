...
Only use this draft route when the user actually wants raw SQL. Console write
mode owns a transaction; do not draft CREATE INDEX CONCURRENTLY for that route.
...
work and Execute write SQL is enabled. One SQL batch runs in one transaction and
commits on success. Never request broad SQL write authority to bypass a narrower
