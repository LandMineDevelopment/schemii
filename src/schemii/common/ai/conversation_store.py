"""Bounded product chat documents; inference context and rows never enter here."""
...
class ConversationStore:
    def __init__(self, factory, policy, product, subject_key="modelId"):
        self.factory, self.policy, self.product, self.subject_key = factory, policy, product, subject_key
        self.lock = RLock()
    ...
    def list(self, owner, subject=None):
        ...
        with self.session(owner) as cursor:
            if cursor is not None:
                ...
                return [row["document"] for row in cursor.fetchall()]
            return [{k:val for k,val in v.items() if k not in {"messages","activity","pending"}} for v in self._all(cursor,owner) if subject is None or v[self.subject_key] == subject]
    ...
    def create(self, owner, value):
        ...
        with self.session(owner) as cursor:
            ...
            if count >= self.policy.maximum_chats_per_workspace:
                raise ApiProblem(409,"ai_chat_limit",f"The configured conversation limit across your {self.product.title()} resources has been reached. Delete an old chat first.",
                    limit_event=LimitEventNotice(f"{self.product}_ai_chat", "ai.maximum_chats_per_workspace", self.policy.maximum_chats_per_workspace, count))
    ...
    def _recover(value):
        if value["status"] == "working":
            value.update(status="failed", pending=None, error="The server restarted during this turn. Inspect the current resource before retrying; actions are never replayed automatically.")
            value["revision"] += 1
