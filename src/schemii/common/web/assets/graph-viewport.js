...
export class GraphViewport {
  constructor({
    ...
    this.touchListeners = {
      pointerdown: event => this.trackTouch(event),
      pointermove: event => this.moveTouch(event),
      pointerup: event => this.endTouch(event),
      pointercancel: event => this.endTouch(event),
      click: event => {
        if (this.suppressTouchClick && event.pointerType !== "mouse") {
  ...
  trackTouch(event) {
    ...
    if (!this.touches.size) this.suppressTouchClick = false;
    this.touches.set(event.pointerId, { x: event.clientX, y: event.clientY });
    if (this.touches.size < 2) return;
    event.preventDefault(); event.stopImmediatePropagation();
    if (this.pinch) return;
    // A second finger converts a pan or uncommitted node drag into a viewport
    // gesture. Cancel the node edit, never save layout changes from a pinch.
    this.cancelNodeDrag();
    if (this.pan) this.endPan({ pointerId: this.pan.pointerId });
    const points = [...this.touches.values()].slice(0, 2);
    const x = (points[0].x + points[1].x) / 2;
    const y = (points[0].y + points[1].y) / 2;
    this.pinch = {
      ids: [...this.touches.keys()].slice(0, 2),
      distance: Math.max(1, Math.hypot(points[1].x - points[0].x, points[1].y - points[0].y)),
      zoom: this.view.zoom, anchor: this.screenToWorld(x, y),
    };
    this.suppressTouchClick = true;
    for (const id of this.touches.keys()) this.host.setPointerCapture(id);
  }

  moveTouch(event) {
    if (!this.touches.has(event.pointerId)) return;
    this.touches.set(event.pointerId, { x: event.clientX, y: event.clientY });
    if (!this.suppressTouchClick) return;
    event.preventDefault(); event.stopImmediatePropagation();
    if (!this.pinch) return;
    const [a, b] = this.pinch.ids.map(id => this.touches.get(id));
    if (!a || !b) return;
    const zoom = clamp(this.pinch.zoom * Math.hypot(b.x - a.x, b.y - a.y) / this.pinch.distance, this.minZoom, this.maxZoom);
    const bounds = this.host.getBoundingClientRect();
    this.setView({ zoom,
      x: (a.x + b.x) / 2 - bounds.left - this.pinch.anchor.x * zoom,
      y: (a.y + b.y) / 2 - bounds.top - this.pinch.anchor.y * zoom });
  }
  ...
  endTouch(event) {
    ...
    // Do not turn the last remaining finger into an accidental pan or click.
  }
  ...
  startPan(event) {
    if (event.button !== 0 || this.pan || this.drag || this.pinch || !this.canStartPan(event)) return;
  ...
  beginNodeDrag(event, {
    ...
    if (this.drag || this.pan || this.pinch || event.button !== 0) return false;
    event.preventDefault();
  ...
  destroy() {
    ...
    for (const [type, listener] of Object.entries(this.touchListeners)) {
      this.host.removeEventListener(type, listener, true);
    }
    for (const [type, listener] of Object.entries(this.listeners)) {
