...
const migrationScene = shell({
  ...
  overlay: `<div class="qs-s-migration-overlay">${miniDialog("Server-authoritative PostgreSQL change", "Review migration", `<p>Review the saved changes against the live PostgreSQL target before authorizing SQL.</p><div class="qs-s-migration-plan"><strong>Ordered PostgreSQL changes</strong><div class="qs-s-migration-change"${target("migration-table-change")}>01 · Create table wishlists <span>⌄</span></div><code class="qs-s-create-sql">CREATE TABLE "bookstore"."wishlists" ("id" bigint NOT NULL, "name" text NOT NULL, "customer_id" bigint, CONSTRAINT "wishlists_pkey" PRIMARY KEY ("id"));</code><div class="qs-s-migration-change"${target("migration-relationship-change")} data-quick-start-tooltip="high">02 · Link wishlists.customer_id <span>⌄</span></div><code class="qs-s-relationship-sql">ALTER TABLE "bookstore"."wishlists" ADD CONSTRAINT "wishlists_customers_fkey" FOREIGN KEY ("customer_id") REFERENCES "bookstore"."customers" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION;</code></div>`, `${button("Close")}${button("Refresh review", "refresh-review")}${button("Apply migration", "apply-migration", true)}`, "qs-s-migration-dialog")}</div><div class="qs-s-confirm-overlay">${miniDialog("Confirm action", "Apply migration", `<p>Apply the two reviewed changes to PostgreSQL? The server runs the displayed SQL in a managed transaction.</p>`, button("Apply migration", "", true), "qs-s-confirm-dialog")}</div>`,
