...
export async function readExecution(response, { maximumRows = 100, timeoutMs = 60000, signal } = {}) {
  ...
  while (["reserved", "running"].includes(execution.status)) {
    if (signal?.aborted || Date.now() > deadline) {
      ...
      throw new Error(signal?.aborted ? "Query cancelled." : "Preview exceeded one minute; cancellation requested. Narrow the query.");
    }
    await new Promise(resolve => setTimeout(resolve, 400));
    execution = await requestJson(base);
  }
  ...
  let page = await requestJson(url), rows = [...page.rows];
  while (page.nextCursor && rows.length < maximumRows) {
    page = await requestJson(`${url}?cursor=${encodeURIComponent(page.nextCursor)}`);
    rows.push(...page.rows);
  }
