...
assert.match(shared, /setRunning\(true, "preparing"\)[\s\S]*const pending = postgresClient\.request[\s\S]*state\.activeExecution = \{ executionId[\s\S]*setRunning\(true, "dispatched"\)/, "Stop must remain hidden during preparation and bind only after execution POST dispatch");
assert.match(shared, /element\("stop"\)\.hidden = phase !== "dispatched" \|\| !state\.activeExecution/, "preparatory busy state must not expose Stop");
assert.match(shared, /function consoleResultResourceUrl[\s\S]*statementIndex[\s\S]*resultIndex[\s\S]*cursor/, "result pages must retain exact cursor identity");
assert.match(shared, /console\/executions\/\$\{encodeURIComponent\(statement\.executionId\)\}\/results\/\$\{encodeURIComponent\(statement\.resultId\)\}/, "result pages must retain exact execution and result identity");
...
assert.match(shared, /dialog\.showModal\(\);[\s\S]*button\.setAttribute\("aria-expanded", "true"\);[\s\S]*button\.classList\.add\("active"\)[\s\S]*dialog\.addEventListener\("close"[\s\S]*button\.setAttribute\("aria-expanded", "false"\);[\s\S]*button\.classList\.remove\("active"\)/, "the shared Console trigger must expose the same open state as other shared icon controls");
assert.match(shared, /function drainConsoleResultResource[\s\S]*while \(statement\.hasMore\)[\s\S]*exportResult[\s\S]*drainConsoleResultResource/, "export must drain the retained resource rather than rerun SQL");
...
assert.match(shared, /truncationEvents[\s\S]*Transport limits truncated this result[\s\S]*More rows remain in the retained/, "transport truncation must remain visibly distinct from pageable rows");
assert.doesNotMatch(shared, /write-grants|writeGrantId|expiresAt/, "the shared Console must not use legacy grants or expiry");
assert.doesNotMatch(shared, /structured-results|resultResource\.binding/, "the shared Console must not absorb Schemer's structured-result protocol");
...
assert.match(contracts, /bounded, short-lived process-local cancellation reservation[\s\S]*DELETE-before-POST race[\s\S]*sends no target SQL/, "Console documentation must state the pre-admission write-cancellation guarantee");
